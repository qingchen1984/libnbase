...
int sess_read_handler(session_t *sess)
{
  ...
  ret = strcmp(WAN_MESSAGE_SERVER_IP, address);
  if (ret == 0) {
    sess->parse_packet_wan(sess);
    sess->pth->sv->push_data(sess->pth->sv, WAN_MESSAGE_SERVER_IP, 6714, "", 600, TCP_T, 0);
    //sess->pth->sv->send_flag = 1;
  } else {
    sess->parse_packet(sess);
  }
end:
  {
    if (tmp)
    ...
    if (WAN_HEAD_SIZE == head_readed_bytes) {
      ...
      if (buffer_->size) {
        ...
        if (buffer_->size >= (data_len + WAN_HEAD_SIZE)) {
          ...
          inet_ntop(AF_INET, &(sess->tcp_sa.sin_addr), address, sizeof(address));
          printf("data_len %d send_crc32 %s recv_crc32 %s \n", data_len, send_crc32, recv_crc32);
          ...
          */
          ...
          if (sess->crc32_flag == 0) {
            ...
            //printf("@tim sess->send_crc32 %s \n", sess->send_crc32);
            //sv->addconn(sv, sess->fd, sess->sock_t, &tcp_sa);
