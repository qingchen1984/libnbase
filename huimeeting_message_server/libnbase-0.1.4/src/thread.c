...
void pth_terminate_session(thread_t *pth, session_t *sess)
{
  ...
  */
  ...
  if (sess && sess->fd < pth->sv->max_connections) {
    ...
    if ((node = tr_->search(tr_, tr_->root, sess->send_crc32)) != NULL) {

      if (node->sockfd == sess->fd) {
        printf("@tim thread delete node sess->send_crc32 %s node->sockfd %d sess->fd %d \n", sess->send_crc32, node->sockfd, sess->fd);
        tr_->delete_node(tr_, &tr_->root, sess->send_crc32);
      }
    }

    printf("@tim thread terminate session sess->send_crc32 %s sess->fd %d \n", sess->send_crc32, sess->fd);
    sess->terminate(sess);
    sess->clean(&sess);
  }
